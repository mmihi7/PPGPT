...
with st.container():
    if selected_document and selected_document != "None":
        ...
        # Create a response area for chat history
        response_area = st.container()
        
        # Display chat history in a non-scrollable area
        with response_area:
            st.markdown("<div style='height: calc(100vh - 300px); overflow-y: hidden;'>", unsafe_allow_html=True)  # Adjust height as needed
            for entry in st.session_state.chat_history:
                if entry['role'] == 'user':
                    ...
                    for char in entry['content']:
                        ...
                        time.sleep(0.01)  # Typing speed of 0.01 seconds per character
