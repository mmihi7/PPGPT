...
if selected_document and selected_document != "None":
    ...
    st.sidebar.subheader("Original Document")
    st.sidebar.markdown(f"[Open Original Document](file://{doc_path})", unsafe_allow_html=True)

    # Read the document content for chat
    ...
    # Summary Accordion
    with st.sidebar.expander("AI-Generated Summary", expanded=False):
        if os.path.exists(summary_path):
            with open(summary_path, "r") as f:
                st.sidebar.markdown(f.read(), unsafe_allow_html=True)
        else:
            st.sidebar.write("Summary not available.")

    # Highlights Accordion
    with st.sidebar.expander("Key Highlights and Impacts", expanded=False):
        if os.path.exists(highlights_path):
            with open(highlights_path, "r") as f:
                st.sidebar.markdown(f.read(), unsafe_allow_html=True)
        else:
            st.sidebar.write("Highlights not available.")
else:
    st.sidebar.write("No document selected")
...
with row1_col1:
    ...
    if selected_document and selected_document != "None":
        ...
        # Read the document content for chat
        doc_content = read_pdf(doc_path)  # Add this line to define doc_content
